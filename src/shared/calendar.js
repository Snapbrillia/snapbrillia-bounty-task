...
// Inspiration from https://github.com/tien271/react-add-to-calendar-recurring
...
const isMobile = () => {
  ...
  const formatTime = (date) => {
    return format(new Date(date.toISOString().slice(0, -1)), "yyyyMMdd'T'HHmmss'Z'");
  };
...
const buildRecurringEvent = (event) => {
  ...
  let recur = `RRULE:FREQ=${event.recurring.repeat};INTERVAL=${
    event.recurring.interval || 1
  };WKST=${event.recurring.weekStart || 'SU'}`;
...
export const buildCalendarUrl = (event, type) => {
  let calendarUrl = '';
  ...
  switch (type) {
    case 'google':
      ...
      if (event.recurring) {
        calendarUrl += '&recur=' + buildRecurringEvent(event);
      }
    ...
    default:
      ...
      if (event.recurring) {
        calendarUrl = calendarUrl.concat([buildRecurringEvent(event)]);
      }
